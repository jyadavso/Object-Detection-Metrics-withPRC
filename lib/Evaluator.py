...
img_path = '/home/jyadavso/sourabh/people_counter_lookdown/Object-Detection-Metrics/detections/'
img_ext = '.jpg'
conf_thresh_for_drawing = 0.4

def draw_bbox(img_path, img_name, bbox, color, conf):

    img_loc = img_path + img_name + img_ext
    #print("img_loc: ", img_loc)
    img = cv2.imread(img_loc, cv2.IMREAD_COLOR)
    str_conf = 'conf: ' + str(conf)
    lt = (int(bbox[0]), int(bbox[1]))
    #rb = (int(bbox[0]) + int(bbox[2]), int(bbox[1]) + int(bbox[3]))
    rb = (int(bbox[2]), int(bbox[3]))
    img_h, img_w = img.shape[:2]
    print('img name:', img_name, ' ', str_conf, " img w, h: ", img_w, ' ', img_h, ' BBox size: ', lt[0], " ", lt[1], ' ', rb[0], ' ', rb[1])
    cv2.rectangle(img, lt, rb, color, 3)
    font = cv2.FONT_HERSHEY_SIMPLEX

    cv2.putText(img, str_conf, (lt[0], lt[1] - 10), font, 0.5, color, 1, cv2.LINE_AA)
...
class Evaluator:
    def GetPascalVOCMetrics(self,
        ...
        for c in classes:
            ...
            """
            # Draw GT Only
            gt_new = [gt_new for gt_new in gts ]
            for k in range(len(gt_new)):
                # print('Ground truth gt => %s' % (gt[j][3],))
                draw_gt_color = (255, 255, 255)
                draw_bbox(img_path, gt_new[k][0], gt_new[k][3], draw_gt_color, 1.0)
            """
            ...
            for d in range(len(dects)):
                ...
                if dects[d][2] > conf_thresh_for_drawing:
                    draw_color = (255,0,0)
                    draw_bbox(img_path, dects[d][0], dects[d][3], draw_color, dects[d][2])
                """

                # Sourabh: This was done to get all the confidence values to further choose between those
                #print("Img name: ", dects[d][0])
                confs.append(dects[d][2])
                ###
                ...
                for j in range(len(gt)):
                    # print('Ground truth gt => %s' % (gt[j][3],))
                    #draw_gt_color = (255, 255, 255)
                    #draw_bbox(img_path, dects[d][0], gt[j][3], draw_gt_color, 1.0)
                ...
                if iouMax >= IOUThreshold:
                    if det[dects[d][0]][jmax] == 0:
                        ...
                        "# Draw TP
                        if dects[d][2] > conf_thresh_for_drawing:
                            draw_bbox(img_path, dects[d][0], dects[d][3], (255, 0, 0), dects[d][2])
                        """
                    else:
                        ...
                        # Draw FP
                        if dects[d][2] > conf_thresh_for_drawing:
                            draw_bbox(img_path, dects[d][0], dects[d][3], (0, 0, 0), dects[d][2])
                        """
            ...
            acc_FP = np.cumsum(FP)
            acc_TP = np.cumsum(TP)
            #print("len acc_TP: ", len(acc_TP), " len dects: ", len(dects))

            rec = acc_TP / npos
            prec = np.divide(acc_TP, (acc_FP + acc_TP))

            #print("len rec: ", len(rec), " len prec: ", len(prec))
            ...
            #print("len mrec: ", len(mrec), " len mprec: ", len(mpre))
